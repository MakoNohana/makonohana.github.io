...
function percent() {
  ...
  if (urlinfo == '/'){
      if (result > 23) {
        document.getElementById("page-name").innerText = "推荐内容".split(" | 茉子")[0];
      }
      if (result > 52) {
        document.getElementById("page-name").innerText = "软件下载".split(" | 茉子")[0];
      }
      if (result > 65) {
        document.getElementById("page-name").innerText = "最新文章".split(" | 茉子")[0];
      }
      if (result > 98) {
        document.getElementById("page-name").innerText = "版权栏".split(" | 茉子")[0];
      }
    }
}
document.getElementById("page-name").innerText = document.title.split(" | 茉子")[0]; 
// 检测按键
